...
def main():
    ...
    for slot,s in specs.items():
        run(art,'short',slot);d=read(art/'short'/slot/'case'/TRACE);data[slot]=d
        if slot.endswith(('_up','_down','_double','_uniform')):
            ...
            factor=np.pi*.029**2*(.25*k['inner_permeability_m2']+.75*k['outer_permeability_m2'])/(s['liquid']['dynamic_viscosity_Pa_s']*.009011660896432553)
            q=factor*pressure;share=.25*k['inner_permeability_m2']/(.25*k['inner_permeability_m2']+.75*k['outer_permeability_m2'])
    ...
    for model,n in [('C',64),('E2',2)]:
        ...
        zones=[]
        for rank in (0,1):
            final=final_directory(art/'short'/(model+'_mpi')/'case'/f'processor{rank}',.2)
            zones.append(sorted(set(scalar_internal_values(final/'permeabilityZoneId',cell_count=32*n//2))))
        if sorted(zones)!=[[0.],[1.]]:raise ValueError('MPI interface not decomposed')
        checks[model+'_mpi_zones']=zones
    write(DOC/'SHORT_CHECKS.json',dict(status='PASS',checks=checks,ledger_sha256=sha(art/'short/INVOCATIONS.jsonl'),started=14,completed=14))
