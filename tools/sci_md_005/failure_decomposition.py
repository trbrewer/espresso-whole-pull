...
def main():
 ...
 for x in manifest:
  ...
  for species in ('caffeine','trigonelline'):
   ...
   compartments[(hypothesis,species)].append({'initial_inventory_kg':initial,'cup_mass_kg':cup,'dissolved_mass_kg':diss,
    'remaining_inventory_kg':remaining,'retained_mass_kg':initial-remaining-cup-diss-back,'back_diffused_mass_kg':back,
    'inventory_utilization':(initial-remaining)/initial,'fraction_initial_remaining':remaining/initial,
 ...
 signs_h0=[x['H0_signed_error_kg_m3'] for x in detail];signs_h1=[x['H1_signed_error_kg_m3'] for x in detail]
 result={'schema_version':'ewp.sci-md-005-consumed-comparison-diagnostic/v1','source':'COMMITTED_SCI_MD_004_ARTIFACTS_ONLY','angeloni_status':'CONSUMED_POST_HOLDOUT_COMPARISON_DATA',
  ...
  'signed_error_distribution':{
   'H0':{'minimum':min(signs_h0),'median':statistics.median(signs_h0),'maximum':max(signs_h0),'mean':statistics.mean(signs_h0)},
   'H1':{'minimum':min(signs_h1),'median':statistics.median(signs_h1),'maximum':max(signs_h1),'mean':statistics.mean(signs_h1)}},
  'every_H1_species_error_same_sign':all(v<0 for v in signs_h1) or all(v>0 for v in signs_h1),'H1_error_sign':'NEGATIVE' if all(v<0 for v in signs_h1) else 'MIXED',
