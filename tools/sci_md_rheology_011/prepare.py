...
def bind(art, frozen=False):
    patterns=['tools/sci_md_rheology_0[01][0-9]/*.py','scripts/*.py',
              ...
              'tests/test_sci_md_rheology_011.py']
    files={p for pattern in patterns for p in ROOT.glob(pattern) if p.is_file()}
    files.update(DOC/n for n in ('PROTOCOL.md','CONTRACT.json','SOURCE_USE.md','REUSE.json','GEOMETRY_CONTRACT.json'))
    files.update(OLD/n for n in ('CONTRACT.json','BUILD.json','REUSE.json','ARTIFACT_RECEIPT.json','FREEZE.json'))
    if frozen:files.update(DOC/n for n in ('SHORT_CHECKS.json','SHORT_QUALIFICATION.json','COMPATIBILITY.json','SOURCE_CHECK.json'))
    ...
    f=dict(files={str(p.relative_to(ROOT)):sha(p) for p in sorted(files)},
           ...
           governance='G2',change_declaration='NO_GOVERNING_PHYSICS_CHANGE',
           planned_native_integrations=46,max_native_attempts=48)
...
def prepare(art,accepted,pw):
    ...
    for k,v in matrix().items():
        s=relocate(oldspec[k]);s['aggregate_viscosity']['table']=str((art/'tables'/table_name(v['law'],v['resolution'])).resolve());science[k]=s
    oldshort=json.loads((accepted/'SHORT_SCENARIOS.json').read_text());short={}
    for model in ('C','E2'):
        for variant in ('up','down','double','uniform','dynamic','repeat','mpi'):
            k=model+'_'+variant;s=relocate(oldshort[k]);
            if variant=='uniform':s['hydraulics']['permeability_profile'].update(inner_permeability_m2=1.3125e-15,outer_permeability_m2=1.3125e-15)
            name=Path(s['aggregate_viscosity']['table']).name
    ...
    for name in ('constant.table','double.table'):
        shutil.copy2(accepted/'tables'/name,art/'tables'/name)
    ...
    write(DOC/'GEOMETRY_CONTRACT.json',interface())
    bind(art)
...
def main():
    ...
    if a.freeze:
        ...
        if q['status']!='PASS' or q['completed']!=14:raise ValueError('short qualification')
        from .run import complete
        specs=json.loads((a.artifacts/'SHORT_SCENARIOS.json').read_text())
        complete(a.artifacts,specs)
        if q['ledger_sha256']!=sha(a.artifacts/'ATTEMPTS.jsonl'):raise ValueError('short ledger changed')
        if json.loads((DOC/'COMPATIBILITY.json').read_text())['status']!='PASS':raise ValueError('legacy compatibility')
