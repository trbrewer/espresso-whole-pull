...
class MappingC1(unittest.TestCase):
 ...
 def test_programme(self):
  p=json.loads((ROOT/"provenance/EXISTING_DATA_LEVERAGE_PROGRAMME.json").read_text());o={x["opportunity_id"]:x for x in p["opportunities"]};self.assertEqual(p["current_priority"],"SCI-DATA-FUSION-001");self.assertEqual(p["current_claim_ceiling"],o[p["current_priority"]]["claim_ceiling"]);self.assertEqual(p["last_completed_opportunity_review"],"XSV-PANNUSCH-EWP-INPUT-MAPPING-001");self.assertEqual(o["XSV-PANNUSCH-EWP-INPUT-MAPPING-001"]["status"],"COMPLETE_NEGATIVE");self.assertEqual(o["SCI-DATA-FUSION-001"]["status"],"READY")
