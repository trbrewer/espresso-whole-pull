...
class XsvPannuschC1(unittest.TestCase):
 ...
 def test_programme_and_protection(self):
  p=json.loads((ROOT/'provenance/EXISTING_DATA_LEVERAGE_PROGRAMME.json').read_text()); x=next(v for v in p['opportunities'] if v['opportunity_id']=='XSV-PANNUSCH-MULTIMODEL-001'); self.assertEqual(x['status'],'COMPLETE_NULL'); self.assertEqual(p['current_priority'],'SCI-MD-012'); self.assertFalse(p['laboratory_gate']['operation_authorized']); self.assertIn('corpus',x['notes'])
  changed=subprocess.check_output(['git','diff','--name-only',BASE,'36b68dc2670349738871ed2aad233357b1474123'],cwd=ROOT,text=True).splitlines(); self.assertFalse(any(x.startswith('solver/') for x in changed)); self.assertFalse(any('angeloni' in x.lower() for x in changed))
