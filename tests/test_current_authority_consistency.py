...
CURRENT_MARKERS = {
    "README.md": (
        ...
        "No\n> solver or surrogate task is active",
        "current scientific gate is direct\n> paired measurement feasibility",
        "Physical validation is **NOT_ESTABLISHED**",
    ),
    "docs/PROJECT_STATE.md": (
        "SCI-MD-009-C2 is `PAUSED_NOT_CURRENT_PRIORITY`",
        "Current solver task: `NONE`. Current\nsurrogate task: `NONE`",
        "paired inventory/species-fraction feasibility",
    ...
    "docs/QA_STATUS.md": (
        "scientific gate is `DIRECT_PAIRED_MEASUREMENT_FEASIBILITY`",
        "active solver\n> and surrogate tasks are `NONE`",
    ...
    "docs/PROGRAM_STATE_AND_FORWARD_PLAN.md": (
        "current-action fields are superseded by",
        "current gate is direct paired\n> measurement feasibility; no solver or surrogate task is active",
    ...
    "docs/strategy/SOLVER_DEVELOPMENT_AND_VALIDATION_ROADMAP.md": (
        "SCI-LC-001A is historical/closed and is not\n> an active successor. SCI-MD-009-C2 is paused",
...
def current_authorities_pass(texts: dict[str, str], qa: dict) -> bool:
    ...
    for key, expected in (
        ...
        ("current_scientific_gate", "DIRECT_PAIRED_MEASUREMENT_FEASIBILITY"),
