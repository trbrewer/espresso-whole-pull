def reduce_overall(components):
    blockers=[c for c in components if c.get("decision_material") and c.get("blockers")];conflicts=[c for c in components if c.get("component_result")=="CONFLICT_SAME_SCOPE_SUPPORTS"];positives=[c for c in components if c.get("component_result")=="POSITIVE_COMMON_CONSTRAINT"];narrowed=[c for c in positives if c.get("narrowing",{}).get("quantitative_narrowing_claim")];common_no_narrow=[c for c in positives if not c.get("narrowing",{}).get("quantitative_narrowing_claim")];complementary=[c for c in components if c.get("component_result")=="COMPLEMENTARY_SOURCE_CONDITIONED_ONLY"]
