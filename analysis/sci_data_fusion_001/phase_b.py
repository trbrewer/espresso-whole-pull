...
from .preexecution import uncertainty_eligible
from .reporting import render_result,render_reproduction
...
def execute(root:Path,output:Path,fixture=None):
    ...
    uncertainties=[{"support_id":item["support_id"],"statistic":item["uncertainty_statistic"],"estimand":item["uncertainty_estimand"],"scale":item["uncertainty_scale"],"replicate_unit":item["replicate_unit"],"observation_operator":item["observation_operator"],"numeric_combination":"PROHIBITED"} for item in supports if uncertainty_eligible(item)];decision=reduce_overall(components);output.mkdir(parents=True,exist_ok=True)
    ...
    authority=json.loads((root/"docs/analysis/sci_data_fusion_001/AUTHORITY.json").read_text());(output/"RESULT.md").write_text(render_result(decision,authority,uncertainties));(output/"REPRODUCTION.md").write_text(render_reproduction(authority));artifacts=[]
