...
The required exact-reproduction gate fails. Frozen SCI-MD-004 H0 did not implement the newly specified H0 identity: its indexed caffeine, trigonelline, residual, and aggregate laws used a common **absolute** `C_sat=180 kg/m3`. An absolute concentration ceiling is not inventory-scaled, so the species and aggregate extracted fractions differ. Across 66 frozen species-condition comparisons, the maximum extracted-fraction discrepancy is 0.10538032195259106 and maximum cup-mass discrepancy is `3.644123943515249e-05 kg`. A single common aggregate fraction therefore cannot reproduce the frozen caffeine, frozen trigonelline, and frozen total-solids values simultaneously.
