...
from tools.validation.val001.framework import (
    assert_invocation_available, assert_real_execution_context, canonical_json,
...
def main() -> None:
    ...
    if authority["status"] != "AUTHORIZED_FOR_SINGLE_CORRECTED_INVOCATION" or activation["status"] != "ACTIVE_FOR_BOUNDED_EXECUTION":
    ...
    ledger_path = root / "validation/val001/VAL_001_INVOCATION_ACCOUNTING_CONTRACT.json"
    ...
    invocation_id = "VAL001-CORRECTED-REAL-001"
    ...
    result = {"schema_version": "espresso.val001.comparison_result.v2", "task": "VAL-001", "classification": "POST_OBSERVATION_REPRODUCTION", "input": {"path": spec["input"]["path"], "sha256": spec["input"]["sha256"], "selected_row_canonical_sha256": spec["input"]["selected_row_canonical_sha256"]}, "execution_counts": {"corrected_real_data_comparison_invocations": 1, "corrected_governed_result_producing_invocations": 1, "corrected_test_or_ci_real_data_invocations": 0, "openfoam_case_executions": activation["actual_openfoam_case_executions"], "fit_or_retune_count": 0, "protected_access_count": 0, "holdout_score_count": 0}, "comparisons": comparisons, "interpretation": {"rules_fired": fired, "decision": decision, "variant_discrimination": "NOT_ESTABLISHED", "physical_equivalence": "NOT_ESTABLISHED", "model_correctness": "NOT_ESTABLISHED", "mechanism_identification": "NOT_ESTABLISHED"}, "claim_boundaries": spec["claim_boundaries"]}
